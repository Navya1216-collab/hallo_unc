...
def save_output_tensor(tensor, filename="frame"):
    # Ensure tensor is on CPU and remove the batch dimension
    tensor = tensor.cpu().squeeze(0)  # Shape: (3, 16, 512, 512)
    
    num_frames = tensor.size(1)  # 16 frames
    height, width = tensor.size(2), tensor.size(3)  # 512 x 512

    # Create an empty array to hold the concatenated image
    grid_size = 4  # We want a 4x4 grid
    concatenated_image = torch.zeros((3, height * grid_size, width * grid_size), dtype=tensor.dtype)

    # Iterate over the frames and place them in the grid
    for t in range(num_frames):
        row = t // grid_size  # Determine the row index
        col = t % grid_size  # Determine the column index
        frame = tensor[:, t, :, :]  # Shape: (3, 512, 512)
        concatenated_image[:, row * height:(row + 1) * height, col * width:(col + 1) * width] = frame

    # Convert to PIL image and save
    pil_image = ToPILImage()(concatenated_image)
    pil_image.save(filename)
        
def save_image_tensor(tensor, filename: str, save_dir: str = "intermediate_outputs"):
    """
    Save a PyTorch tensor or a list of tensors as images in the specified directory.

    Args:
        tensor (torch.Tensor or list): The image tensor(s) to save.
        filename_prefix (str): The filename prefix for the saved image (e.g., "source_image_pixels").
        save_dir (str): Directory to save the images. Default is "intermediate_outputs".
    """
    # Ensure the directory exists
    os.makedirs(save_dir, exist_ok=True)

    # Check if tensor is a list and iterate if necessary
    if isinstance(tensor, list):
        for idx, t in enumerate(tensor):
            save_single_tensor_image(t, f"{filename}_{idx}.png", save_dir)
    else:
        save_single_tensor_image(tensor, f"{filename}.png", save_dir)

def tensor_to_image(tensor):
    # Move tensor to CPU and convert to numpy
    image_np = tensor.squeeze().cpu().detach().numpy()
    # Normalize and scale to 0-255 for uint8 format
    image_np = (image_np - image_np.min()) / (image_np.max() - image_np.min()) * 255
    return image_np.astype(np.uint8)



def save_single_tensor_image(tensor: torch.Tensor, filename: str, save_dir: str):
    """
    Helper function to save a single tensor as an image.
    ...
    pil_image = to_pil(tensor.cpu().squeeze(0))  # Remove batch dimension if it exists
...
def inference_process(args: argparse.Namespace):
    ...
    # reference_unet_image = tensor_to_image(reference_unet[0])
    # Image.fromarray(reference_unet_image).save(os.path.join(output_dir, "reference_unet_output.png"))

    # denoising_unet_image = tensor_to_image(denoising_unet[0])
    # Image.fromarray(denoising_unet_image).save(os.path.join(output_dir, "denoising_unet_output.png"))

    # face_locator_image = tensor_to_image(face_locator[0])
    # Image.fromarray(face_locator_image).save(os.path.join(output_dir, "face_locator_output.png"))

    # image_proj_image = tensor_to_image(image_proj[0])
    # Image.fromarray(image_proj_image).save(os.path.join(output_dir, "image_proj_output.png"))
    ...
    # save_image_tensor(source_image_pixels, "image_pixels")
    # save_image_tensor(source_image_full_mask, "source_image_mask")
    # save_image_tensor(source_image_face_mask, "face_mask")
    # save_image_tensor(source_image_face_region, "face_region")
    # save_image_tensor(source_image_lip_mask, "lip_mask")
    
    # save_image_tensor(source_image_face_emb, "face_emb.png")
    ...
    for t in range(times):
        ...
        # save_image_tensor(pixel_values_ref_img, "reference_image")
        ...
        # save_output_tensor(pipeline_output.videos, filename=f"frame_{t}.png")
...
if __name__ == "__main__":
    ...
    # Streamlit setup
    st.title("Hallo: Hierarchical Audio Driven Visual Synthesis")
    st.write("Upload an input image and an audio file to generate a talking face animation.")

    # File uploader widgets
    image_file = st.file_uploader("Upload Source Image", type=["jpg", "png", "jpeg"])
    ...
    # Load files only if both are uploaded
    if image_file and audio_file:
        # Display uploaded files
        st.image(Image.open(image_file), caption="Source Image", use_column_width=True)
        st.audio(audio_file, format="audio/wav")
        pil_image = Image.open(image_file)
        # st.image(pil_image, caption="Verified Image")
        ...
        pil_image_save_path = os.path.join(output_dir, "input_image.jpg")
        pil_image.save(pil_image_save_path, format="JPEG")

        with open(audio_path, "wb") as f:
            f.write(audio_file.read())

        # Run the inference process when the "Generate Animation" button is clicked
        if st.button("Generate Animation"):
            with st.spinner("Processing... Please wait."):
                # Set up arguments
                args = argparse.Namespace(
                    config="/home/beast/supari/hallo/configs/inference/default.yaml",  # Update with actual config path
                ...
                try:
                    inference_process(args)  # Replace with your function name
        ...
        st.info("Please upload both an image and an audio file.")